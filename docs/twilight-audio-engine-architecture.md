...
- 退役解码流（gapless 自动切歌、手动 next、重新 preload 时被换下的 `DecodeStream`）同样走 epoch ACK 回收：control thread 撤下 render 指针后递增 `retiredStreamEpoch_`，render callback 在读取流指针前采样该值、在每条退出路径末尾（`recordRenderPerformance`）发布为 `renderAckedStreamEpoch_`；control/clock thread 只回收 `epoch <= ACK` 的流（Stopped 时全部回收），join 解码线程与释放 FFmpeg/DsdReader/环形缓冲从不发生在音频回调内。此前回收要等整个 pipeline 进入 Stopped，整张专辑连播期间每首都常驻一个解码器加 2 s 环形缓冲。回归用例：`testRetiredDecodeStreamsAreReclaimedWhilePlaying`（`twilight_runtime_queue_reroute_unit`）。
...
- 库内 Track/Album 批量分析由 `audio/libraryLoudnessManager.ts` 编排，按完整专辑逐组提交到同一 analysis pool，种类为 `loudness-batch`、优先级 −20、执行 deadline 14,520 秒。取消该种类不会中断自动 loudnorm 或 BPM；完整 Album 由 libebur128 合并测量状态，不能平均 dB。CUE 从源头解码跳过精确的前缀样本，排除虚拟 PREGAP。每组的测量与文件身份在最终检查后同步原子写入 `userData/library-loudness/`；取消检查与提交之间没有异步窗口。指纹覆盖算法 2、模式、成员 ID / 顺序、canonical path、size、mtime、实际源区间，查询会重新验证。失败或取消的当前组不提交半套专辑；完成组可跨重启读取。首版只做库内结果管理，既有播放增益及标签写回路径保持独立。详见 [批量响度接口](./audio-engine-api.md#库内批量响度分析)。
- ASIO 兼容层不携带 SDK，只在 Windows x64 构建中编译并默认枚举已安装驱动；排障或紧急回退可通过 `TWILIGHT_DISABLE_ASIO=1` 显式禁用。
- miniaudio 0.11.25 已作为 Windows Shared/default PCM provider PoC 编译依赖接入；通用 `TAE_ENABLE_MINIAUDIO` 默认仍关闭，但 Windows MinGW 发布 preset 打开它，`TAE_DEFAULT_PCM_PROVIDER=legacy` 则把未设置 runtime selector 的编译期默认保持为 legacy。`TWILIGHT_AUDIO_PCM_PROVIDER=legacy|miniaudio` 只在 control path 为公开 `wasapi` Shared backend 选择实现，`legacy` 也是显式 rollback 值；非法值和 build-disabled 请求明确失败而不回退，WASAPI Exclusive、ASIO 与 DSD 特殊路由不受影响。MA-101 固定 Float32 callback 并关闭 WASAPI `AUTOCONVERTPCM`，从 miniaudio converter/device state 区分实际转换事实，通知通过 control event path 延迟派发。Windows PCM 目录对两种实现统一使用 endpoint stable ID；公开 JSON 增量提供 `platformStableId`、`providerFamily`、`defaultRole`、`lastKnownLabel`，raw `ma_device_id` 只存在于一次 adapter 枚举/open 会话。miniaudio 禁止 WASAPI 自动换路由，default role 也固定解析到当下 endpoint；main 检测到 active `auto` 默认端点变化后重放完整 backend/device/config route transaction，并要求 `actualDeviceId` 匹配新观察到的 endpoint 后才 commit。显式设备过期、重复或在 open 前消失时直接失败，不回退系统默认。构建可用、bounded A/B 和真实设备采用是分开的证据维度，当前不能据此宣称 G3/G4 完成。

...
| store                                       | 拥有状态                                           | 要点                                                                                                                                                                                                                                                                      |
| ------------------------------------------- | -------------------------------------------------- | ------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------- |
| `usePlayerStore.ts`（约 147KB，最大 store） | 播放队列、当前曲目、播放状态、音频输出、会话恢复   | 播放 tick/统计用 `shallowRef` + `triggerRef`；不要整表复制；纯逻辑已抽到 `utils/playerTime.ts`、`playerAudioSettings.ts`、`playerQueueUtils.ts`、`playerConstants.ts` 及既有 `utils/player*`；队列/会话/歌词/播放时钟/播放历史/音频输出/心动模式控制器在 `stores/player/` |
...
队列编辑通过 `playbackQueueController.ts` 接入 `queueCommandController.ts` 的增量撤销与 revision；`playbackSelectionController.ts` 统一普通选曲和按位置播放，`utils/nativeQueueTrackIndex.ts` 解析原生队列的具体重复项。命名会话由 `queueWorkspaceStore.ts` 串行持久化，`queueSessionSources.ts` 收集当前来源，`queueSessionRestore.ts` 解析可恢复队列，`queueSessionController.ts` 处理取消、选择与显式播放。`playbackSessionController.ts` 共用暂停选曲与运行状态重置；实际播放顺序由 `playbackHistoryController.ts` 记录，`QueueWorkspaceDialog.vue` 提供会话管理和虚拟历史列表。跨进程契约归 `shared/queueWorkspace.ts`，main 归 `ipc/queueWorkspaceIpc.ts`。
...
`libraryScanService`（独立 utilityProcess）枚举 + 解析 metadata/封面 → `libraryIndexCoordinator` 持久化 `path + size + mtime` 快速索引 → 启动只做增量核对；全量 metadata/封面重扫必须由用户显式触发（进度/暂停/取消）。大扫描通过有界批次传输 identity 与解析结果，进度按时间/文件数节流；单文件解析和扫描 worker 均有 watchdog，worker 重启后跳过当前阻塞路径并在下次扫描重试。提交前重查 revision、授权 roots、exclusions，drift 则丢弃重规划。renderer 先加载已保存曲库让 UI 可用，后台再合并增量；大增量提交后由 renderer 重新加载已原子提交的文档。
...
| `stores/usePlayerStore.ts`                               | 约 147KB | 最大 store；纯逻辑已抽到 `utils/playerTime.ts`、`playerAudioSettings.ts`、`playerQueueUtils.ts`、`playerConstants.ts` 及既有 `utils/player*`；Round 3/4/5 已抽队列/会话/歌词/播放时钟/播放历史控制器到 `stores/player/`，Round 6 已抽 `audioOutputController.ts`，Round 7 已抽 `heartModeController.ts`；024 D 提取 `audioOutputState.ts`，A 提取 `playbackSelectionController.ts` / `utils/nativeQueueTrackIndex.ts`，队列命令和命名会话实现在独立控制器中；后续继续按统计域拆分 |
